...
bool EvseManager::powersupply_DC_set(double _voltage, double _current) {
    ...
    // Enforce EVSE maximum current limit from energy management
    {
        auto hlc_limits = charger->get_evse_max_hlc_limits();
        if (current > hlc_limits.evse_maximum_current_limit && hlc_limits.evse_maximum_current_limit > 0) {
            current = hlc_limits.evse_maximum_current_limit;
        }
        // Also enforce power limit
        if (hlc_limits.evse_maximum_power_limit > 0 && voltage > 0) {
            double power_limited_current = hlc_limits.evse_maximum_power_limit / voltage;
            if (current > power_limited_current) {
                current = power_limited_current;
            }
        }
    }
